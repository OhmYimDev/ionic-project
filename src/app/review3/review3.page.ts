...
@Component({
  ...
  templateUrl: './review3.page.html',
  styleUrls: ['./review3.page.scss'],
})
export class Review3Page {
  ...
  { 
    this.review.comment = '';
    this.review.score = '';
    this.review.id_attr = 5;
    this.loaddata(5);  
    this.dataUser();
    this.loaddata_score(5);
    this.load_countreviews(5);
    this.review.name_attr = "วัดโพธารามมหาวิหาร";
  }
  ...
  insertdata(){
    ...
    if (this.review.username != '' && this.review.comment != '') 
    {
      ...
      data.subscribe(data =>{
        this.ShowAlert();
        this.navCtrl.navigateBack("/photharam");
