...
@Component({
  selector: 'app-review8',
  templateUrl: './review8.page.html',
  styleUrls: ['./review8.page.scss'],
})
export class Review8Page {
  ...
  { 
    this.review.comment = '';
    this.review.score = '';
    this.review.id_attr = 10;
    this.loaddata(10);  
    this.dataUser();
    this.loaddata_score(10);
    this.load_countreviews(10);
    this.review.name_attr = "อุทยานหลวงราชพฤกษ์";
  }
  ...
  insertdata(){
    ...
    if (this.review.username != '' && this.review.comment != '') 
    {
      ...
      data.subscribe(data =>{
        this.ShowAlert();
        this.navCtrl.navigateBack("/ratchapruek");
