...
@Component({
  selector: 'app-review6',
  templateUrl: './review6.page.html',
  styleUrls: ['./review6.page.scss'],
})
export class Review6Page {
  ...
  { 
    this.review.comment = '';
    this.review.score = '';
    this.review.id_attr = 8;
    this.loaddata(8);  
    this.dataUser();
    this.loaddata_score(8);
    this.load_countreviews(8);
    this.review.name_attr = "พิพิธภัณฑสถานแห่งชาติเชียงใหม่";
  }
  ...
  insertdata(){
    ...
    if (this.review.username != '' && this.review.comment != '') 
    {
      ...
      data.subscribe(data =>{
        this.ShowAlert();
        this.navCtrl.navigateBack("/museum");
